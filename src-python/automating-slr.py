...
# Remove the following if-else block when the next release("WebUI v2.4.3") is out on https://github.com/webui-dev/webui/releases(the required change is already in the "main" branch)
...
def filter():
    ...
    filtering_feedback : FilteringFeedback = bib_to_csv(bib_files, min_pages,max_pages, start_year, end_year, publication_types)
    # clear the cache
    del bib_files
    gc.collect()
    return jsonify(filtering_feedback) 
